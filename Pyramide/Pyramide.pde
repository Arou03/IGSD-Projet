...
void initPyramide(int SIZE) {
  labyrinthe = new char[SIZE][SIZE];
  sides = new char[SIZE][SIZE][4];
  int todig = 0;
  ...
  labyrinthe[SIZE-2][SIZE-1] = ' '; // sortie
...
void keyPressed() {
  ...
  if (anim==0 && keyCode==40  && inLab && labyrinthe[posY-dirY][posX-dirX]!='#') {
    posX-=dirX; 
    posY-=dirY;
  }
